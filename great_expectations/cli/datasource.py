...
def datasource_list(directory):
    ...
    try:
        ...
        if failed and context is not None:
            send_usage_message(
                data_context=context, event="cli.datasource.list", success=False
            )
...
"""
    ...
    if data_source_location_selection == "1":
        ...
        datasource_name = _add_sqlalchemy_datasource(
    ...
    if passthrough_generator_only:
        ...
        if prompt_for_datasource_name:
            datasource_name = click.prompt(
                msg_prompt_datasource_name, default=datasource_name, show_default=True
            )
  ...
  - Database Error: {0:s}"""
        try:
            ...
            if not click.confirm(
                "Enter the credentials again?".format(str(de)), default=True
            ):
                context.add_datasource(
                    datasource_name,
                    initialize=False,
...
Name the new expectation suite"""
    ...
    existing_suite_names = [
        expectation_suite_id.expectation_suite_name
...
"""
    ...
    if batch_kwargs_generator_name is not None:
        ...
        if option_selection == "1":
            ...
            generator_asset_selection = click.prompt(
                prompt, default=None, show_default=False
            )
    ...
    while True:
        path = click.prompt(
            msg_prompt_file_path,
            type=click.Path(
                exists=True, file_okay=True, dir_okay=dir_okay, readable=True
            ),
            show_default=True,
            default=path,
        )
...
"""
    ...
    if datasource.engine.dialect.name.lower() == "snowflake":
        ...
        temp_table_kwargs = {"snowflake_transient_table": table_name}
    elif datasource.engine.dialect.name.lower() == "bigquery":
        # bigquery also requires special handling
        table_name = click.prompt(
            "GE will create a table based on your query to use for "
            "validation." + os.linesep + "Please enter a name for this table: ",
            default="ge_tmp_" + str(uuid.uuid4())[:8],
            show_default=True,
        )
        temp_table_kwargs = {"bigquery_temp_table": table_name}
