...
class PandasDatasource(Datasource):
    ...
    def get_batch(self, batch_kwargs, batch_parameters=None):
        ...
        if (
            PY2
            and "reader_options" in batch_kwargs
            and "sep" in batch_kwargs["reader_options"]
            and batch_kwargs["reader_options"]["sep"] is not None
        ):
            batch_kwargs["reader_options"]["sep"] = str(
                batch_kwargs["reader_options"]["sep"]
            )
        ...
        if df.memory_usage().sum() < HASH_THRESHOLD:
            batch_markers["pandas_data_fingerprint"] = hashlib.md5(
