...
class ExpectationOperator(BaseOperator):
    ...
    def _setup_source_conn(self, source_conn_id, source_bucket_name=None):
        ...
        if not isinstance(self.source_conn, DbApiHook) and not isinstance(
            self.source_conn, S3Hook
        ):
            raise AttributeError(
                "Only s3_csv, local and sql connection types are allowed, not {0}".format(
                    type(self.source_conn)
                )
            )
    ...
    def _get_dataframe(self):
        ...
        if isinstance(self.source_conn, S3Hook):
            ...
            return hook.get_ge_df(
                self.dataset_name, self.source_bucket_name, **self.dataset_params
            )
    ...
    def _load_json(self):
        ...
        if os.path.isfile(self.expectations_json):
            self.log.info(
                "Loading expectation suite from file {file}".format(
    ...
    def _store_results(self, results):
        ...
        return hook.dump_results(
            results, self.results_bucket_name, self.results_dest_name
        )
    ...
    def execute(self, context):
        ...
        self.log.info(
            "Start dataset validation for set {set}".format(set=self.dataset_name)
        )
            ...
            if result.success is False:
                if self.fail_on_error is True:
                    raise AirflowException(
                        "Validation failed for dataset {name}".format(
