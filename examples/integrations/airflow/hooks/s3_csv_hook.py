...
class ExpectationS3CsvHook(S3Hook):
    def get_ge_df(self, dataset_name, bucket_name=None, **kwargs):
        if not self.check_for_key(dataset_name, bucket_name):
            (
                ...
                region_name,
                s3_endpoint_url,
            ) = self._get_credentials("eu-west-1")
            self.log.info(aws_access_key_id)
            self.log.info(aws_secret_access_key)
            raise AirflowException(
                "The source key {0} does not exist in bucket {1}".format(
        ...
        with NamedTemporaryFile("w") as temp_file:
            self.log.info(
                "Temp dumping S3 file {0} contents to local {1} file".format(
    ...
    def dump_results(self, results, bucket_name, dest_file_name):
        ...
        with NamedTemporaryFile("w") as temp_file:
            self.log.info(
                "Temp dumping expectation results to local file {temp_file}".format(
                    temp_file=temp_file
                )
            )
            ...
            self.log.info(
                "Uploaded {temp} to bucket {bucket} with key {key}".format(
                    temp=temp_file, bucket=bucket_name, key=dest_file_name
                )
            )
